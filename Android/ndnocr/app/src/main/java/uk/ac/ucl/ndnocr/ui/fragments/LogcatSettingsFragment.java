...
public class LogcatSettingsFragment extends ListFragment {
  ...
  public boolean onOptionsItemSelected(MenuItem item)
  {
    ...
    if(id==R.id.action_log_settings) {
  ...
  public void onListItemClick(ListView l, View v, int position, long id) {
    final String logTag = m_logcatSettingItems.get(position-1).getLogTag();
  ...
  public void onActivityResult(int requestCode, int resultCode, Intent data) {
    ...
    case REQUEST_CODE_DIALOG_SET_LOG_LEVEL:
      ...
      if (listPosition != -1) {
        m_logcatSettingItems.get(listPosition-1).setLogLevel(newLogLevel);
