...
Forwarder::onIncomingData(Face& inFace, const Data& data)
{
  ...
  for (const shared_ptr<pit::Entry>& pitEntry : pitMatches) {
    ...
    NFD_LOG_DEBUG("inrecord");
      if (inRecord.getExpiry() > time::steady_clock::now()) {
    NFD_LOG_DEBUG("inrecord2");
    ...
    NFD_LOG_DEBUG("Insert dead nonce");
    // Dead Nonce List insert if necessary (for OutRecord of inFace)
    this->insertDeadNonceList(*pitEntry, true, data.getFreshnessPeriod(), &inFace);
    NFD_LOG_DEBUG("deleteInRecords");
    // mark PIT satisfied
    pitEntry->deleteInRecords();
    pitEntry->deleteOutRecord(inFace);
    NFD_LOG_DEBUG("setStragglerTimer");
    // set PIT straggler timer
    this->setStragglerTimer(pitEntry, true, data.getFreshnessPeriod());
  }

    NFD_LOG_DEBUG("setStragglerTimer2");
  ...
  for (Face* pendingDownstream : pendingDownstreams) {
    ...
    NFD_LOG_DEBUG("no pending dowstreaam");
}
